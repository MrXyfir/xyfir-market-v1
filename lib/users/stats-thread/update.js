  ...
  try {
    ...
    const stats = {
      ...
      verifiedProfiles: user.verifiedProfiles
        ? JSON.parse(user.verifiedProfiles)
        : null,
      feedback: {
        seller: {
          positive: 0,
    ...
    stats.feedback.seller.positive = rows.reduce(
      (sum, row) => (row.userType == 2 && row.type == 1 ? sum + 1 : sum),
      0
    );
    stats.feedback.seller.negative = rows.reduce(
      (sum, row) => (row.userType == 2 && row.type == -1 ? sum + 1 : sum),
      0
    );
    stats.feedback.buyer.positive = rows.reduce(
      (sum, row) => (row.userType == 1 && row.type == 1 ? sum + 1 : sum),
      0
    );
    stats.feedback.buyer.negative = rows.reduce(
      (sum, row) => (row.userType == 1 && row.type == -1 ? sum + 1 : sum),
      0
    );
    ...
    stats.feedback.seller.list = rows.filter(r => r.userType == 2).slice(0, 30);
    stats.feedback.buyer.list = rows.filter(r => r.userType == 1).slice(0, 10);
