...
module.exports = async function(r, message, thread) {
  ...
  try {
    ...
    let rows = await db.query('SELECT author FROM sales_threads WHERE id = ?', [
      thread
    ]);
    ...
    await message.reply(rows.map(r => r.item).join('\n\n'));
  } catch (err) {
    if (typeof err != 'string')
      return console.error('commands/thread/autobuy/list', err);
